//! Палитра NES: 64 цвета (54 уникальных) → RGB888.
//!
//! PPU отдаёт индекс цвета 0..63. Эта таблица — стандартная палитра NES
//! (взята из runes::bin, которая в свою очередь из wiki.nesdev.com).

/// 64 RGB-цвета NES (по 24 бита на запись: R<<16 | G<<8 | B).
...
/// Преобразовать индекс цвета NES (0..63) в (R, G, B). Без bounds-check —
/// индекс маскируется до 6 бит (на случай мусора).
#[inline(always)]
