...
impl Dma {
  ...
  pub fn start(&self, config: u32, src: u32, dest: u32, n: u32) {
    ...
    // D-cache работает write-back, а DMAC читает DDR напрямую и кэш не видит.
    // Выталкиваем грязные строки: и этот дескриптор, и буфер-источник (его
    // заполнял CPU). Без этого DMAC прочитает устаревшую память — дескриптор
    // с нулями (перенос не пойдёт) или прошлый кадр на дисплее.
    // `dcache.call` чистит кэш целиком, поэтому одного вызова тут достаточно
    // на оба буфера.
    crate::cache::clean_dcache();
