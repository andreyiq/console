...
impl Display {
  ...
  /// То же, но во время ожидания зовёт `on_wait`.
  ///
  /// Нужно для звука: перенос кадра занимает ~2 мс, и если в это время никто
  /// не откачивает буфер сэмплов, в звуке появляется провал на каждом кадре —
  /// призвук на частоте кадров. Кто именно откачивает, дисплею не важно,
  /// поэтому это замыкание, а не вызов `speaker::pump()` напрямую.
  pub fn flush_region_dma_finish_polling(&self, mut on_wait: impl FnMut()) {
    let dma = dma::Dma::Channel0;
    while dma.en_read() & 1 != 0 {
      on_wait();
    }
    while !self.spi.burst_done() {
      on_wait();
    }
    self.spi.cs_high();
  }
