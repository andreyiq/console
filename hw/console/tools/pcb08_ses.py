...
Кладём заново каждый раз: скрипт сначала снимает с платы медь трассировки —
всё, кроме закреплённых лучей из-под F133, — а потом кладёт то, что пришло. Так его можно гонять сколько
...
def clear(board):
    """Снять медь трассировки, не тронув наши лучи из-под F133.

    Лучи помечены блокировкой (`pcb07_fanout.py`) и в ответе не приходят:
    неприкосновенную медь freerouting обратно не отдаёт, он возвращает только
    то, что проложил сам. Снеси их — и половина работы пропадёт молча.
    """
