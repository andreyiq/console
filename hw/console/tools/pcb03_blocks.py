...
#
# Все детали на ЛИЦЕ, под панелью и в полях вокруг неё. План построен от того,
# куда смотрит каждая банка выводов F133 — посчитано по координатам площадок:
#
#   влево  — хранение `PF`/`PC` (7…19) и такт (20…30)
#   вверх  — питание и USB (101…117), все семнадцать
#   вниз   — отладка `PE` (31…45) и половина шины дисплея
#   вправо — вторая половина `PD` и аудио (87…100)
#
# Габарит панели — X 36…120, Y 9.7…64.3, просвет под ней 2.0 мм. Всё, что выше
# 1.8 мм (разъёмы, гребёнки, кнопки), стоит вне этого прямоугольника; зоны
# ниже целиком помещаются под стекло.
ZONES = [
    # Зарядник — в правый нижний угол, между розеткой USB и разъёмом банки:
    # через него идут оба его конца, `VBUS` от `J301` и `BAT` к `J1`. Стоял в
    # левом верхнем — обе цепи бежали через всю плату, по девяносто миллиметров
    # каждая, при токе заряда 1 А.
    ("зарядник",    2, lambda r: r not in BUCKS and r not in SEQ,
                                                 118.0, 52.0, 146.0, 64.0),
    ("секвенсор",   2, lambda r: r in SEQ,         3.0, 20.0,  12.0, 26.0),
    
    ("хранение",    4, None,                     110.0, 12.0, 130.0, 28.0),
    ("такт",        7, None,                     112.0, 49.0, 128.0, 60.0),
    ("отладка",     9, None,                      80.0, 10.0, 110.0, 16.0),
...
def to_front(fp):
    """Все детали живут на лице — под панелью и в полях вокруг неё."""
    if fp.IsFlipped():
