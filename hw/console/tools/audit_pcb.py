...
PANEL_GAP = 1.4                             # просвет под панелью
...
def check_battery(board):
    """3. Детали в зоне банки на изнанке."""
    n, bad = 0, []
    for f in parts(board):
        if not f.IsFlipped() or f.GetReference().startswith("H"):
            continue
        n += 1
        if hits(box(f), BATT):
...
def check_under_panel(board):
    """4. Детали на лице под панелью, кроме разъёма шлейфа.

    Просвет под панелью 1.4 мм (10-mech.md §4), туда влезает только сам
    разъём высотой 1.0. Всё остальное на лице обязано быть за габаритом
    панели, иначе стекло встанет на деталь.
    """
    n, bad = 0, []
    for f in parts(board):
        if f.IsFlipped() or f.GetReference().startswith("H"):
            continue
        n += 1
        if f.GetReference() == "J601":
            continue
        if hits(box(f), PANEL):
            bad.append(f.GetReference())
    return n, bad
...
CHECKS = [
    ...
    ("деталей на изнанке", "в зоне банки", check_battery),
    ("деталей на лице", "под панелью", check_under_panel),
